# ADR-0030 — Scheduler/worker CLI, propagation de job_id, câblage best-effort `/ingest/v2` (LOT44e)

- **Statut** : Accepté
- **Date** : 2026-08-04
- **Décideur** : Alaeddine Ben Rhouma (Shark)
- **Découle de** : ADR-0027 (plan de contrôle PostgreSQL, LOT44b), ADR-0028 (profils et validation déterministe, LOT44c — « Contrat d'interface pour LOT44d et LOT44e »), ADR-0029 (stages déterministes, LOT44d)
...
`QUALITY_CHECKED -> ROUTED` reste non activée (héritage direct d'ADR-0029, Décision 3 — non rouvert ici).

## Décision 4 — `pii_detected`/`duplicate_detected` restent `False` en dur dans le worker

Aucun détecteur réel n'existe dans ce dépôt (cf. ADR-0029, Décision 5) : le worker ne fabrique aucune détection, il transmet toujours `False` — documenté explicitement dans `runner.py`, pas une omission silencieuse.
