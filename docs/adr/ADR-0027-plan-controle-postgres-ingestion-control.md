# ADR-0027 — Plan de contrôle PostgreSQL `ingestion_control` (LOT44b, décision D1)

- **Statut** : Accepté
- **Date** : 2026-08-04
- **Décideur** : Alaeddine Ben Rhouma (Shark)
- **Découle de** : ADR-0001 (séparation plan de contrôle / plan de données / cockpit), ADR-0002 (contrat partagé `nexus-contracts` versionné), ADR-0026 (contrats canoniques d'ingestion, LOT44a)
...
- Un ADR dédié reste nécessaire avant toute réintroduction d'un état `PUBLISHED` ou d'un comportement de publication (déjà acté par ADR-0026).
