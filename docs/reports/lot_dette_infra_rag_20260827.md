...
| 29 | **Valeur de visibilité hors contrat dans un fichier d'évaluation** : `retrieval_metadata_eval.yml` emploie `visibility: student_visible` et `rights: allowed_for_retrieval`, absents du `Literal["public", "internal", "restricted", "private"]` du contrat. Même famille que le reste : un vocabulaire parallèle qui ne se confronte à rien. Le fichier porte `status: metadata_only_eval` et `real_documents_allowed: false`, donc il n'écrit nulle part — mais une évaluation qui note contre un vocabulaire que le contrat ignore mesure autre chose que ce qu'elle annonce | moyenne |
