...
@dataclass(frozen=True)
class VerifiedReleaseScopePlacement:
...
class _BlobReader(Protocol):
    input_blob_sha256: dict[str, str]
...
class _FrozenBlobReader:
    """Lecteur strict de blobs déjà gelés par la frontière appelante."""

    def __init__(self, source_blobs: Mapping[str, bytes]) -> None:
        self._source_blobs: dict[str, bytes] = {}
        self.input_blob_sha256: dict[str, str] = {}
        for path, raw in source_blobs.items():
            canonical = _canonical_repo_path(path)
            if canonical != path:
                raise _fail("INVALID_REPO_PATH", f"non-canonical source path {path!r}")
            if not isinstance(raw, bytes):
                raise _fail("INVALID_INPUT", f"source blob {path!r} is not bytes")
            self._source_blobs[canonical] = raw

    def read_blob(self, path: str) -> bytes:
        canonical = _canonical_repo_path(path)
        try:
            raw = self._source_blobs[canonical]
        except KeyError as exc:
            raise _fail("MISSING_SOURCE_BLOB", f"missing exact source blob {canonical!r}") from exc
        self.input_blob_sha256[canonical] = hashlib.sha256(raw).hexdigest()
        return raw
...
def produce_release_scope_placement_from_git(
    *,
    ...
    placement, profile_facts = _produce_release_scope_placement(
        reader=reader,
...
def produce_release_scope_placement_from_blobs(
    *,
    ...
    """Produit et vérifie la projection depuis les seuls octets fournis."""
    ...
    return VerifiedReleaseScopePlacement(
        placement=placement,
        verified_profile_facts=profile_facts,
        input_blob_sha256=dict(sorted(reader.input_blob_sha256.items())),
    )
