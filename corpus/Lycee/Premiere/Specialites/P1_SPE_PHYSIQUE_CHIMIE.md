...
- Horaire : 4 h/semaine.
- Collection cible : `rag_nexus_pc_premiere_specialite` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Constitution et transformations de la matière** : Suivi temporel d'une transformation chimique, Modélisation microscopique et macroscopique, Réactions acide-base, Réactions d'oxydoréduction.
- **Mouvement et interactions** : Description du mouvement : vecteurs, Lois de Newton, Interactions fondamentales et champs.
- **Ondes et signaux** : Ondes progressives et périodicité, Lumière : modèles ondulatoire et particulaire.
- **Énergie : conversions et transferts** : Conversions d'énergie, Transferts thermiques.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/physique_chimie/premiere_specialite.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=premiere`, `voie=generale`, `matiere=pc`, `statut_enseignement=specialite`, `session` (declared_or_null), `audience` (libre/aefe/tous).
