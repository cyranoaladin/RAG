...
- Horaire : 4 h/semaine.
- Collection cible : `rag_nexus_hggsp_premiere_specialite` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Histoire** : Ruptures et continuités : le multilatéralisme depuis 1945, Mémoires et commémorations.
- **Géographie** : Aires et mers : enjeux géostratégiques, Environnement et développement.
- **Science politique** : La démocratie : fondements et régimes.
- **Regards croisés** : Information, médias et opinions publiques.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/hggsp/premiere_specialite.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=premiere`, `voie=generale`, `matiere=hggsp`, `statut_enseignement=specialite`, `session` (declared_or_null), `audience` (libre/aefe/tous).
