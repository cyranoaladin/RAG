...
- Collection cible : `rag_nexus_lv_premiere_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Axes du programme (cycle terminal)** : L'art et le pouvoir, Espace privé et espace public, Identités et échanges.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/langues/premiere.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=premiere`, `voie=generale`, `matiere=langues`, `statut_enseignement=tronc_commun`, `session` (declared_or_null), `audience` (libre/aefe/tous).
