...
- Horaire : 6 h/semaine.
- Collection cible : `rag_nexus_hggsp_terminale_specialite` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_8_2019-07-25)
- **Histoire** : Puissances et aires d'influence.
- **Géographie** : Marges : intégration et exclusion.
- **Science politique** : L'État-providence, Ordre politique et légitimité.
- **Regards croisés** : Le patrimoine : mémoire, protection, valorisation.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/hggsp/terminale_specialite.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=terminale`, `voie=generale`, `matiere=hggsp`, `statut_enseignement=specialite`, `session` (declared_or_null), `audience` (libre/aefe/tous).
