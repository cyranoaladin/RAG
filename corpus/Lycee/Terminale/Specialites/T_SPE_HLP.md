...
- Horaire : 6 h/semaine.
- Collection cible : `rag_nexus_hlp_terminale_specialite` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_8_2019-07-25)
- **Thèmes officiels** : Le métier de citoyen, Le monde d'aujourd'hui, Agir dans le monde : représentation, information, communication.
- **Méthodes** : Analyse de textes littéraires et philosophiques, Expression écrite et orale argumentée.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/hlp/terminale_specialite.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=terminale`, `voie=generale`, `matiere=hlp`, `statut_enseignement=specialite`, `session` (declared_or_null), `audience` (libre/aefe/tous).
