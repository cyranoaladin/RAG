...
## 1. Identité
- Niveau : **terminale** — voie générale — EMC.
- Horaire : Environ 18 h/an.
- Collection cible : `rag_nexus_emc_terminale_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_8_2019-07-25)
- **Normes et démocratie** : Normes, règles et jugement moral, La démocratie à l'épreuve.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/emc/terminale.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=terminale`, `voie=generale`, `matiere=emc`, `statut_enseignement=emc`, `session` (declared_or_null), `audience` (libre/aefe/tous).
