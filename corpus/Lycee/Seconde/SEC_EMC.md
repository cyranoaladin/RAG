...
## 1. Identité
- Niveau : **seconde** — voie générale — EMC.
- Horaire : Environ 18 h/an (0,5 h/semaine).
- Collection cible : `rag_nexus_emc_seconde_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Être citoyen** : Les pouvoirs publics et l'action collective, La laïcité en débat, Formes de l'engagement citoyen.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/emc/seconde.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=seconde`, `voie=generale`, `matiere=emc`, `statut_enseignement=emc`, `session` (declared_or_null), `audience` (libre/aefe/tous).
