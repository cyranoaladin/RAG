...
- Horaire : 3 h/semaine.
- Collection cible : `rag_nexus_pc_seconde_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Constitution et transformations de la matière** : Du macroscopique au microscopique, Solutions et concentration, Réactions physiques et chimiques, modélisation.
- **Ondes et signaux** : Ondes mécaniques et propagation, Lumière : spectres et vision, Signaux et capteurs.
- **Mouvement et interactions** : Description du mouvement, Actions mécaniques et forces.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/physique_chimie/seconde.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=seconde`, `voie=generale`, `matiere=pc`, `statut_enseignement=tronc_commun`, `session` (declared_or_null), `audience` (libre/aefe/tous).
