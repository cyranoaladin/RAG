...
- Horaire : 5 h 30/semaine (LVA + LVB).
- Collection cible : `rag_nexus_lv_seconde_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_1_2019-01-22)
- **Axes du programme (cycle terminal)** : Sauver la planète, Représentation de soi et rapport à autrui, Identités et échanges.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/langues/seconde.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=seconde`, `voie=generale`, `matiere=langues`, `statut_enseignement=tronc_commun`, `session` (declared_or_null), `audience` (libre/aefe/tous).
