...
- Horaire : 4 h/semaine (LVA + LVB, toutes langues confondues).
- Collection cible : `rag_nexus_lv_troisieme_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_11_2018-07-26_aj_2020)
- **Axes culturels (cycle 4)** : Fantaisie et fantastique, Progrès et rêves scientifiques, Engagement citoyen, Rencontre avec d'autres cultures.
- **Compétences langagières** : Compréhension de l'oral, Compréhension de l'écrit, Expression orale en continu et en interaction, Expression écrite.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/langues/troisieme.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=troisieme`, `voie=college`, `matiere=langues`, `statut_enseignement=tronc_commun`, `session` (declared_or_null), `audience` (libre/aefe/tous).
