...
- Horaire : 1 h 30/semaine (dans l'enseignement de technologie cycle 4).
- Collection cible : `rag_nexus_techno_troisieme_tc` (ADR-0015 — convention rag_nexus_{matiere}_{niveau}_{voie}_{statut}).

## 2. Programme officiel (BOEN_special_11_2018-07-26_aj_2020)
- **Algorithmique et objets connectés** : Conception d'algorithmes, Programmation d'objets connectés, Interfaces homme-machine.
- **Design et innovation** : Design et prototypage, Modélisation 3D.
- **Technologie et société** : Techniques et développement durable, Habitats et ouvrages.
...
## 6. Notes RAG / corpus
- Taxonomie : `services/rag-pedago/taxonomy/technologie/troisieme.yml` (thèmes → notions → compétences).
- Indexer par **thème puis notion** ; métadonnées obligatoires : `niveau=troisieme`, `voie=college`, `matiere=techno`, `statut_enseignement=tronc_commun`, `session` (declared_or_null), `audience` (libre/aefe/tous).
