...
from rag_pedago.governance import reference_programme as rp
...
# --- Positions de citation (NEXUS-CITATION-SCOPE-ATTRIBUTION-V1) -------------


def test_la_carte_dindex_ramene_chaque_caractere_a_son_origine():
    decompose = unicodedata.normalize("NFD", "Éléments évalués")
    aplati, origine = rp.aplatir_avec_index(decompose)

    assert aplati == "Elements evalues"
    assert len(origine) == len(aplati)
    # Chaque caractère aplati désigne un caractère réel du texte d'origine.
    for i in range(len(aplati)):
        assert 0 <= origine[i] < len(decompose)
    assert origine == sorted(origine)


def test_les_positions_designent_la_citation_dans_le_texte_dorigine():
    texte = (
        "Programme de première\n"
        "défini par le Bulletin officiel spécial n° 1 du 22 janvier 2019.\n"
    )
    (citation,) = rp.citations_localisees(texte)
    assert citation["official_reference"] == "BOEN_special_1_2019-01-22"
    assert texte[citation["start"]:citation["end"]] == (
        "Bulletin officiel spécial n° 1 du 22 janvier 2019"
    )


def test_positions_justes_sur_un_texte_deja_decompose():
    # Le défaut que ce test existe pour empêcher : sur du texte décomposé —
    # ce que rend couramment une extraction PDF — chaque accent qui précède la
    # citation la décalerait d'un caractère. La citation resterait
    # correctement reconnue, et serait rattachée à la mauvaise section sans
    # qu'aucun symptôme ne le signale.
    texte = unicodedata.normalize(
        "NFD", "Éléments évalués à l'épreuve\nBO spécial n° 1 du 22 janvier 2019"
    )
    (citation,) = rp.citations_localisees(texte)
    assert texte[citation["start"]:citation["end"]] == unicodedata.normalize(
        "NFD", "BO spécial n° 1 du 22 janvier 2019"
    )


def test_deux_occurrences_de_la_meme_reference_restent_deux():
    # `citations_officielles` déduplique — c'est son rôle. La localisation ne
    # doit PAS le faire : deux mentions dans deux sections sont l'information.
    texte = (
        "Seconde : voir le BO spécial n° 1 du 22 janvier 2019.\n"
        "Première : voir le BO spécial n° 1 du 22 janvier 2019.\n"
    )
    localisees = rp.citations_localisees(texte)
    assert len(localisees) == 2
    assert localisees[0]["start"] < localisees[1]["start"]
    assert rp.citations_officielles(texte) == ["BOEN_special_1_2019-01-22"]


def test_localisation_et_citations_officielles_voient_le_meme_ensemble():
    texte = (
        "Le B.O. spécial n° 8 du 25 juillet 2019, le BO n° 44 du 26-11-2015 et "
        "le Bulletin officiel spécial n° 11 du 26 novembre 2015."
    )
    assert sorted({c["official_reference"] for c in rp.citations_localisees(texte)}) == sorted(
        rp.citations_officielles(texte)
    )
