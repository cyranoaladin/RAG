...
import pytest
from nexus_contracts import ResourceRegistryBootstrapPayload, seal_resource_registry_bootstrap
...
def _build(registry=None, specs=None):
    return build_servable_corpus_manifest(
        resource_registry=registry or _registry(_resource()),
...
def test_manifest_is_deterministic_and_preserves_canonical_resource_identity() -> None:
    ...
    assert manifest.resource_registry_sha256 == _registry(_resource()).inventory_sha256
