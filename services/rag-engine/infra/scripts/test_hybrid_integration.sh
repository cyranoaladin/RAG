...
REVOKE ALL PRIVILEGES ON TABLE rag_schema_migrations FROM $PGVECTOR_APP_USER;
...
GRANT SELECT ON TABLE rag_schema_migrations TO $PGVECTOR_APP_USER;
CREATE ROLE $PGVECTOR_REVIEW_USER
    LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS;
REVOKE ALL PRIVILEGES ON SCHEMA public FROM $PGVECTOR_REVIEW_USER;
REVOKE ALL PRIVILEGES ON TABLE rag_chunks FROM $PGVECTOR_REVIEW_USER;
REVOKE ALL PRIVILEGES ON TABLE rag_schema_migrations FROM $PGVECTOR_REVIEW_USER;
GRANT CONNECT ON DATABASE $PGVECTOR_DB TO $PGVECTOR_REVIEW_USER;
GRANT USAGE ON SCHEMA public TO $PGVECTOR_REVIEW_USER;
GRANT SELECT ON TABLE rag_chunks TO $PGVECTOR_REVIEW_USER;
GRANT SELECT ON TABLE rag_schema_migrations TO $PGVECTOR_REVIEW_USER;
GRANT UPDATE (review_status) ON TABLE rag_chunks TO $PGVECTOR_REVIEW_USER;
SQL
}
