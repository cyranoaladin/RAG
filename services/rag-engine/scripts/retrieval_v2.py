...
Chemin de service v2 : resolve_collection_v2 → dense e5-large → rerank CrossEncoder → seuil.
...
# Seuil rerank fondé sur la marge BB-03 : plus bas in-domain (+2.99) vs plus haut
# hors-domaine (+1.51), milieu de marge = +2.25. 15/15 in conservé, 10/10 out rejeté.
...
RERANK_SCORE_THRESHOLD = float(os.environ.get("RERANK_SCORE_THRESHOLD", "2.25"))
...
if not PG_DSN:
    raise RuntimeError("PG_RAG_DSN required")
...
def search(query: str, collection: str, top_k: int = 5) -> list[dict]:
    ...
    # Validate collection via v2 resolver
    cfg = load_collection_config()
    resolve_collection_v2(collection, cfg)
    ...
    pairs = [(query, (c[6] or "")[:512]) for c in candidates]
    rerank_scores = reranker.predict(pairs)

    # Step 3: Filter by seuil + sort
    results = []
    for candidate, score in sorted(zip(candidates, rerank_scores, strict=False), key=lambda x: x[1], reverse=True):
