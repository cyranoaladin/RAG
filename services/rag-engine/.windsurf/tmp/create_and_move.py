...
def move_documents(source_collection, target_collection, drive_folder_id):
    ...
    while True:
        ...
        # Éviter les doublons
        new_ids = [doc['id'] for doc in docs if doc['id'] not in [d['id'] for d in all_docs]]
        if not new_ids:
            break
            
        all_docs.extend([doc for doc in docs if doc['id'] in new_ids])
...
if create_collection(target_collection):
