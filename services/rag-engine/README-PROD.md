...
- Fournir deux identifiants PostgreSQL runtime distincts : `PG_RAG_DSN` pour un rôle strictement `SELECT` et `PG_REVIEW_DSN` pour un rôle limité à `SELECT` plus `UPDATE(review_status)`. `/health` vérifie les privilèges effectifs des deux rôles, et le pool de retrieval impose en plus `default_transaction_read_only=on`. Le moteur refuse de retomber sur `DATABASE_URL_SYNC`, réservé au propriétaire et aux migrations.
